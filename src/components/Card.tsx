...
export function Card({
  ...
  return <>
    {items.map((item, index) => <div key={index} className={` ${isVisible ? 'animate-fade-in' : ''}`}>
      ...
      <div className="flex gap-6 items-start">
        ...
        <div className="flex-1">
          <h2 className={`text-2xl font-semibold text-secondary mb-2 ${useVisible ? 'animate-typing' : ''}`}>{item.title}</h2>
